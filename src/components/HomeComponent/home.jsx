...
function HomeComponent() {
    ...
    return (
        <Common  dummyData={dummyData} >
            ...
            {posts.map(post => (
                <Card className= {`${classes.card} ${classes.additionalClass}`} key={post.postId}>
                <CardContent>
                {post.postId === 1 && (
                    <img src={waterlooImage} alt="Waterloo" />
                )}
                    <Typography color="textSecondary" gutterBottom>
                    <span style={{ fontWeight: 'bold', color: 'black' }}>User ID:</span>
                    ...
                    <span style={{ fontWeight: 'bold', color: 'black' }}>Created At:</span>
